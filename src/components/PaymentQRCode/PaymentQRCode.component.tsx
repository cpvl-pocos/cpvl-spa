...
export const PaymentQRCode: React.FC<PaymentProps> = ({
  ...
  const getDiscount = (type: string): number => {
    if (type !== 'anual') return 0;
    const today = new Date();
    return today.getMonth() === 0 ? 0.1 : 0; // 10% discount in January
  };

  const calculateFinalValue = (type: string): number => {
    const baseValue = getPaymentValue(type);
  ...
  const handlePaymentNotice = async () => {
    ...
    const yearToUse = selectedYear === 'all' ? new Date().getFullYear().toString() : selectedYear;

    try {
      for (let i = 1; i <= numMonths; i++) {
        let month = ref_month_start + i;
        let year = parseInt(yearToUse);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto animate-in fade-in duration-700">
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-md bg-white/95 backdrop-blur-2xl border-none shadow-2xl rounded-[2rem]">
                  ...
                  <div className="flex flex-col items-center gap-4">
                    <div className="relative p-5 bg-white rounded-[2.5rem] shadow-xl border-4 border-slate-50 ring-1 ring-slate-100 group">
                      <QRCodePix
                        ...
                        amount={currentValue} size={200} bgColor="#FFFFFF" level="H"
                  ...
                  <div className="flex flex-col justify-center space-y-6">
                    <div className="space-y-3">
                      ...
                      <Button
                        ...
                        disabled={!pixCode}
                      >
                        <div className="flex items-center gap-3">{copied ? <CheckCircle2 size={18} /> : <Copy size={18} />}<span>{copied ? 'Copiado!' : 'Copiar Código'}</span></div>
                        <ArrowRight size={16} />
                      </Button>
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-100">
                      ...
                      >
                        ...
                        <span className="font-black text-lg relative z-10 flex items-center gap-2">{isSubmitting ? <><Truck className="animate-bounce" />Enviando...</> : 'Confirmar Pagamento'}</span>
                        {!isSubmitting && <span className="text-[10px] uppercase font-bold tracking-widest opacity-70 relative z-10">Avisar administração</span>}
                      </Button>
                      {notice && !isModalOpen && <p className={cn("text-center text-xs font-bold animate-in fade-in", notice.includes('Erro') ? "text-red-500" : "text-green-600")}>{notice}</p>}
                    </div>
                  </div>
                </div>
              ) : (
